...
import { Router, Switch, BrowserRouter, Route, Redirect, Link } from 'react-router-dom';
    ...
    render() {
        return (
            <Switch>
                ...
                <Route exact path='/login' render={(props) => (this._login_page())}/>
                <Route exact path='/home' render={(props) => (this._home())}/>
                <Route path='*' exact={true} render={(props) => ( <Redirect to="/"/>)}/>
            </Switch>
        );
    }
}
